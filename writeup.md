...
The primary obstacles I had to face was different lighting conditions, shadows. During these issues, I had to adjust the threshold parameters to get the pipeline to perform well on the video. Another issue I have faced was the noisy areas that generated by the binary image, and this get includes the transformed image. I was able to tune the parameters in thresholding to reduce this noise; however, there may be more ways to solve this issue in the future. The four points we are locating on the image to perform the transformation is fixed for every image in this project, and this allows the introduction of the noise to the image especially in the curves. It is possible to use more advanced image processing techniques to detect the four source points in every image.  Furthermore, we can use thresholding dynamic to each image rather using fixed thresholding for all the frames. I am hoping to introduce these advanced techniques to improve this project.

# Code Base

Code for this project is in different python files. The following list show where to find the different part of the pipeline in the code base

- Main pipeline code is in the [lanefind.py](lanefind.py)
- Camera calibration code is in the [calibrateCam.py](calibrateCam.py)
- Apply Distortion Correction to each image code is in the [calibrateCam.py](calibrateCam.py)
- Thresholding code is located in the [thresholding.py](thresholding.py)
- Perspective transformation code is located in the [perspective_transform.py](perspective_transform.py)
- Lane line detection using sliding window method is located in the [locatelanelines.py](locatelanelines.py)
